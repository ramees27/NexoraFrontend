...
import { logOut, useget } from '../../api/authapi';
...
const Navbar = () => {
  ...
  useEffect(() => {
    const checkCounselor = async () => {
      try {
        const responce = await useget("/Councelor/CheckStatus")
        console.log(responce.data)
        setVerify(responce.data)
      }
      catch (error) {
        console.log(error)
      }
  ...
  const checkverify = () => {
    if (verify) {
      if (verify.exists && verify.isVerified && !verify.isDeleted) {
        navigate("/counselor/maintabe");
      } else if (verify.exists && !verify.isVerified && !verify.isDeleted) {
        toast.warning("Your application is still under review. We’ll notify you once verified.");
      } else if (verify.exists && verify.isVerified && verify.isDeleted) {
        toast.warning("🚫 Your counselor account has been blocked. Please contact support.");
      }
    }
  };
  return (
                ...
                <div ref={dropdownRef} className="relative">
                  ...
                  {dropdownOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-md shadow-lg z-50">
                      ...
                      {verify?.exists && (
                        <a className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" onClick={checkverify}>Login as Counselor</a>)}
